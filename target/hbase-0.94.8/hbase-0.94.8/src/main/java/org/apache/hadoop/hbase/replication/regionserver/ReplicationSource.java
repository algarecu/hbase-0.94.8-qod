...
public class ReplicationSource extends Thread
   ...
   * @return
   */
  private String entriesArrayToStr(Entry[] entries) {
      String str = "";
      ...
      for (Entry entry : entries) {
          ...
          for (KeyValue kv : kvs) {
              str += ",(\nROW: " + Bytes.toString(kv.getRow()) + ", FAMILY: " + Bytes.toString(kv.getFamily()) + ", QUALIFIER: "
                      + Bytes.toString(kv.getQualifier()) + ", VALUE: " + Bytes.toString(kv.getValue()) + ", TIMESTAMP: "
                      + kv.getTimestamp() + ")";
          }
      }
      if ("".equals(str))
          return "ARRAY CONTENTS: EMPTY";
      else
          return "ARRAY CONTENTS: /n" + str.substring(1);
  }

  /**
   * A.G: check for entries we want to replicate or not, based on VFC cube
   * replication policy.
   * 
   * @param <updates>
   * @throws IOException
  ...
  private Entry[] filterEntriesToReplicate(HLog.Entry[] updates, String containerId) {
      ...
      for (Entry update : updates) {
          List<KeyValue> kvs = update.getEdit().getKeyValues();
          WALEdit newWALEdit = new WALEdit();

          for (KeyValue kv : kvs) {

              String tableName = Bytes.toString(update.getKey().getTablename());
      ...
      System.out.println("Leaving filtering method, filtered edits size: " + lst_edits.size());
      return (Entry[]) lst_edits.toArray(new Entry[lst_edits.size()]);
